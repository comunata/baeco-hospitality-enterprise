...
export default async function AdminSettingsPage() {
  ...
  const [flags, bookingSettings] = await Promise.all([getModuleFlags(), getBookingSettings()]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-sm border border-platinum/10 bg-graphite/60 p-6 text-sm text-stone">
        ...
        <p className="mt-1">Check-in {siteConfig.checkIn} · Check-out {siteConfig.checkOut} · Moneda {siteConfig.currency}</p>
      </div>
    </div>
  );
}
